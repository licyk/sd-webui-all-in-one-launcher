...
main_menu_status() {
  ...
  case "$check_status" in
    ...
    1)
      ...
      status_hint='下一步: 可以先修改当前安装器配置，然后选择 "下载安装器并运行"。'
      ;;
    2)
      status_label="安装状态: 安装目录存在，但缺少管理脚本"
      status_hint='下一步: 当前安装不完整，请选择 "下载安装器并运行" 完成安装。'
      ;;
    *)
...
基本思路
  这个启动器负责选择项目、保存安装参数、下载安装器并运行 PowerShell 脚本。
  它不会把安装器脚本固定在仓库里；每次运行安装器都会重新下载到缓存目录，
  这样可以尽量使用上游最新脚本。

首次使用流程
  1. 进入 "选择不同类型的安装器"，选择要安装或管理的项目。
  2. 进入 "当前安装器配置"，确认安装路径、分支、镜像、代理等参数。
  3. 返回主界面，选择 "下载安装器并运行" 开始安装。
...
主界面顶部状态
  当前安装器: 显示当前选择的项目。若显示 "未选择"，需要先选择安装器。
...
主菜单入口
  选择不同类型的安装器
    切换当前项目。选择后会保存到主配置，下次启动继续使用。

  下载安装器并运行
    每次都会重新下载安装器到缓存目录，然后执行它。
...
常见情况
  ...
  安装器运行失败:
    查看 PowerShell 输出。常见原因是网络不可用、代理/镜像配置不正确、缺少 pwsh。
