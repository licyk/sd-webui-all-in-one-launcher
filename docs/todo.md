...
- [x] GUI 运行安装器或管理脚本时只锁定一键启动和两个卸载入口，设置页日志、文件夹、更新检查和快捷方式操作保持可用。
- [x] GUI 运行安装器前改用可滚动 WPF 确认窗口，长下载源和 PowerShell 参数不会把窗口撑出屏幕。
- [x] `install.ps1` 下载前会自包含配置代理，默认自动读取 Windows 系统代理，并支持命令行临时指定手动代理或关闭代理。
- [x] GUI 启动器卸载 worker 改为从临时目录启动，并在删除文件前结束父启动器和同脚本路径的残留 PowerShell 进程，避免卸载目录被占用。
...
- [x] `install.sh` 会将已获取的临时源码传给自安装流程复用，避免重复下载启动器源码。
...
- [x] 优化 `install.sh` 临时源码复用后运行 `bash -n install.sh installer_launcher.sh lib/*.sh`，通过。
- [x] 优化 `install.sh` 临时源码复用后运行 `shellcheck install.sh installer_launcher.sh lib/*.sh`，通过。
- [x] 验证 `install-launcher --yes` 在收到 `INSTALLER_LAUNCHER_BOOTSTRAP_SOURCE` 时会直接安装已有源码，不再重新下载。
...
- [x] 根据 `installer_docs.md` 破坏性同步 GUI/TUI 热补丁参数：移除旧 `-Hotpatcher` / `HOTPATCHER`，改为 `-DisableHotpatcher`、`-HotpatcherConfig`、`-HotpatcherPort`、`-EnableHotpatcherRuntime` 配置、保存和传递。
- [x] GUI 项目配置读取改为只保留当前注册表支持的 installer / 管理脚本参数，旧参数键不再随 JSON 配置继续保留。
...
- [x] 同步热补丁参数后运行 Bash 参数构造烟测，确认 installer 与 `launch.ps1` 会传递热补丁参数，且 `sd_trainer_script` installer 已按文档启用热补丁参数。
...
- [x] 同步热补丁参数后运行 GUI 参数构造烟测，确认 installer 与 `launch.ps1` 会传递新热补丁参数且不会再传旧 `-Hotpatcher`。
...
- [ ] 在 Windows 中验证 GUI 长路径支持提示：已启用不提示、未启用提示、批准 UAC 后写入成功、取消 UAC 后继续运行。
