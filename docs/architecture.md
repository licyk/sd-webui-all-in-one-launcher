...
9. 后台加载/下载头图和图标；启动完成后检测 Windows 长路径支持，未启用时提示用户并可通过 UAC 提权写入 `LongPathsEnabled=1`。
10. 长路径检测结束后，按自动更新间隔触发非致命更新检查，避免启动提示和更新提示重叠。

GUI 启动器自身卸载会先生成临时 PowerShell worker，并从系统临时目录启动它。worker 开始后会等待或强制结束父启动器进程，同时结束命令行中仍引用同一个 GUI 脚本路径的残留 PowerShell 实例，再删除 GUI 脚本、配置、日志、缓存、快捷方式和卸载注册项。这样卸载目录不会因为原启动器工作目录或脚本文件仍被占用而残留。
...
5. 用户中途停止搜索时，如果本次搜索已经发现结果，会立即展示这些中途结果，但不写入配置。
6. 用户选择候选项后，写入 `CURRENT_PROJECT` 和对应项目 `INSTALL_PATH`，再刷新状态和脚本列表。
