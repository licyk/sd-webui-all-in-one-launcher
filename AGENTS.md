...
- GUI release artifacts are generated by `tools/compile_gui.py`; do not manually concatenate files or edit `dist/installer_launcher_gui.ps1`.
  - Source development stays multi-file under `gui/`.
  - Compiler usage, source constraints, release flow, and known pitfalls are documented in `docs/gui-compiler.md`.
  - Release, install, and GUI self-update URLs must point at the compiled single-file release asset.
  - The compiler embeds `gui/xaml/*.xaml` as Base64 resources; `Load-GuiXamlWindow` must keep the bundled-resource fallback before filesystem XAML lookup.
  - Strip UTF-8 BOM from embedded XAML resources. Windows PowerShell 5.1 can fail `[xml]` conversion when a decoded bundled XAML string starts with BOM.
  - Source multi-file mode has no bundled XAML resource table and must skip GUI self-update to avoid replacing the development entry script with the release artifact.
...
- GUI self-update must remain Windows-only and must not add Bash shell command registration or shell rc cleanup. It updates the compiled single-file release artifact, not the multi-file source tree.
