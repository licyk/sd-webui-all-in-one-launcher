...
GUI 版支持项目选择、动态安装器配置、搜索当前系统中已经安装的 WebUI 并切换管理路径、重新下载安装器并执行、安装检测、管理脚本运行、项目卸载、日志、代理模式、关于页和 GUI 脚本自动更新。首次启动会显示用户协议，只有同意后才进入主界面；同意结果会写入主配置，下次启动不再提示。它不会注册 Bash 命令，也不会执行 Linux/macOS 依赖引导或 shell rc 清理。

GUI 启动时会写入当前用户级 Windows 卸载入口，因此可以在系统“应用和功能”/控制面板中找到 `SD WebUI All In One Launcher` 并启动卸载。卸载会移除 GUI 脚本本体、桌面和开始菜单快捷方式、配置目录以及日志/缓存目录。
...
1. 首次启动时阅读并同意用户协议；不同意会直接退出 GUI。
2. 进入左侧“软件选择”，选择要安装或管理的 WebUI / 工具。
3. 进入“高级选项”，确认安装路径、安装器参数和管理脚本参数；如果系统中已经有安装好的 WebUI，可以在“安装路径”页搜索并选择要管理的路径；需要卸载当前项目时也在安装路径页操作。
4. 返回“一键启动”，启动器会根据安装状态自动切到“安装模式”或“启动模式”；未安装时点击右下角统一启动按钮运行安装器。
5. GUI 会重新下载安装器，并打开独立 PowerShell 控制台运行脚本。
6. 安装完成后，“一键启动”会自动切到“启动模式”，在脚本列表中选择 `launch.ps1`、`update.ps1`、`terminal.ps1` 等脚本运行。
7. 启动器设置位于左下角“设置”，可调整自动更新、日志等级和代理模式；修改后会自动保存，也可以检查更新、查看日志、打开配置文件夹、创建桌面/开始菜单快捷方式和卸载 GUI 启动器。
8. 左侧“关于”页可查看版本、项目说明和用户协议全文。
...
Windows GUI 会定时自动刷新安装状态；安装完成、目录移动或卸载后无需手动点击刷新按钮。
