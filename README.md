...
一个用于管理 `sd-webui-all-in-one` 系列 PowerShell 安装器的 Bash TUI/CLI 启动器。

它负责选择安装器类型、保存安装参数、重新下载最新安装器脚本、执行安装任务，并在安装后运行项目生成的管理脚本。
...
1. 进入 `选择不同类型的安装器`，选择要安装或管理的项目。
2. 进入 `当前安装器配置`，检查安装路径、分支、镜像、代理等参数。
3. 返回主界面，选择 `下载安装器并运行`。
4. 安装完成后，使用 `运行安装后生成的管理脚本` 启动、更新或维护项目。

## 安装启动器命令

可以将启动器安装到用户目录，并注册 `installer-launcher` 命令：
...
- `未选择`：还没有选择当前安装器，需要先选择项目。
- `未安装`：安装路径不存在，可以先调整配置再运行安装器。
- `安装目录存在，但缺少管理脚本`：目录存在但不像完整安装结果，建议重新运行安装器。
- `已安装`：找到了管理脚本，可以运行启动、更新或维护操作。

主菜单常用入口：

- `选择不同类型的安装器`：切换当前管理的项目。
- `下载安装器并运行`：重新下载安装器并执行安装。
- `卸载当前已安装软件`：删除当前项目安装目录，需要双确认。
- `运行安装后生成的管理脚本`：执行安装目录中的管理脚本。
...
- `launch.ps1`：启动项目。运行前会提示可按 `Ctrl+C` 终止。
- `update.ps1`：更新项目。
- `terminal.ps1`：进入项目 Python 环境。退出时输入 `exit` 并回车。
- `settings.ps1`：管理项目设置。
