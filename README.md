...
GUI 版支持项目选择、动态安装器配置、重新下载安装器并执行、安装检测、管理脚本运行、项目卸载、日志、代理模式、关于页和 GUI 脚本自动更新。首次启动会显示用户协议，只有同意后才进入主界面；同意结果会写入主配置，下次启动不再提示。它不会注册 Bash 命令，也不会执行 Linux/macOS 依赖引导或 shell rc 清理。
...
3. 进入“高级选项”，确认安装路径、安装器参数和管理脚本参数；需要卸载当前项目时也在安装路径页操作。
