...
1. 进入左侧“软件选择”，选择要安装或管理的 WebUI / 工具。
2. 进入“高级选项”，确认安装路径、安装器参数和管理脚本参数；需要卸载当前项目时也在安装路径页操作。
3. 返回“一键启动”，启动器会根据安装状态自动切到“安装模式”或“启动模式”；未安装时点击右下角统一启动按钮运行安装器。
4. GUI 会重新下载安装器，并打开独立 PowerShell 控制台运行脚本。
5. 安装完成后，“一键启动”会自动切到“启动模式”，在脚本列表中选择 `launch.ps1`、`update.ps1`、`terminal.ps1` 等脚本运行。
6. 启动器设置位于左下角“设置”，可调整自动更新、日志等级和代理模式；修改后会自动保存，也可以检查更新、查看日志、打开配置文件夹、创建桌面/开始菜单快捷方式和卸载 GUI 启动器。
